...
export default function BookPage() {
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      setIsDownloaded(downloads.some((b: any) => b.id === book.id));
    }
  }, [book]);

  // Fix hydration issue: run orientation check client-side only
  useEffect(() => {
  ...
  const handleDownload = () => {
    ...
    if (!downloads.some((b: any) => b.id === book.id)) {
      const newDownload = {
        id: book.id,
        title: book.title,
      ...
      toast({
        ...
        description: `"${book.title}" has been saved to My Downloads.`,
        duration: 3000,
      });
    }
  };
  ...
  const handleRemoveDownload = () => {
    ...
    const stored = localStorage.getItem("downloads");
    if (stored) {
      const downloads = JSON.parse(stored).filter((b: any) => b.id !== book.id);
      localStorage.setItem("downloads", JSON.stringify(downloads));
      setIsDownloaded(false);

      toast({
        title: "Removed",
        description: `"${book.title}" has been removed from My Downloads.`,
        duration: 3000,
      });
    }
  };
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white text-black">
      <header className="w-full bg-[#219653] p-3 shadow-md">
            ...
            <div className="flex gap-2 ml-auto">
              <Link href="/downloads">
                <Button
                  variant="outline"
                  size="sm"
                  className="bg-white text-blue-700 border-none"
                >
